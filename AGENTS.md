...
- Poll your own open PRs: five minutes after a push, then ~30 minutes. With
  no subscription the check is the only thing that reports, and the first
  one lands early because a push that Codex never picked up looks exactly
  like one it is still reading — see the verdict rule for what to do about
  it. Afterwards the slow cadence is enough. Never end a turn idle with one
  of yours open: arm the next check with whatever the client offers
