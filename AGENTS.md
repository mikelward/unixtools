...
- **Deferring a real-but-out-of-scope finding.** Don't ask the maintainer to
  ...
  nothing back; escalate if the re-review re-raises it, or is still missing
  five minutes after the poke.
...
- Read the Codex verdict, don't infer it. It reacts to the PR body
  ...
  leaves the `codex` status pending — comment `@codex review`, once; if that
  has not landed five minutes on, escalate rather than poking again.
