...
- **Deferring a real-but-out-of-scope finding.** Don't ask the maintainer to
  ...
  push re-triggers Codex, so poke only as *Read the Codex verdict* allows;
  escalate if the re-review re-raises it.
...
- Read the Codex verdict, don't infer it. It reacts to the PR body
  ...
  fixed, rebutted, or deferred (see *Deferring a finding* above); an
